...
Calendar.prototype = {
    ...
    init: function () {
        ...
        el.on("click", ".calendar-cell", function (e) {
            ...
            $(".dropdown-address").removeClass("open");
            // $(".dropdown-address").removeClass("open");
        ...
        //地址下拉
        el.on("click", ".calendar-address-select", function (e) {
            e.stopPropagation();
            $(".dropdown-address").toggleClass("open");
            $(".dropdown-year").removeClass("open");
            $(".dropdown-month").removeClass("open");
            //创建下拉数据
            ...
            for (var i = 0; i < 4; i++) {
                //突出显示
                if (i == 0) {
                    s += '<li class="address-item active">'
                }
                else {
                    s += '<li class="address-item">'
                }
                s += '<span class="address-check">' +  mycars[i] + '</span>'
                s += '<span ></span>'
                s += '</li>'
            }
            me.el.find(".dropdown-address").html(s);
        })
        //地址改变
        el.on("click", ".address-item", function (e) {
            e.stopPropagation();
            // $(".dropdown-address").removeClass("open");
            // var monthText = $(this).text();
            // var monthNum = monthText.split("月")[0];
            ...
            // }
            // me._refreshCalendar(opts.newDate);
            // $(".calendar-address-text").text(monthText);
    ...
    _createToolbar: function () {
        ...
        //設置地址
        s += '<div class="calendar-select calendar-address-select" >'
        s += '<span class="calendar-address-text"> 一号门</span >'
        s += '<span class="calendar-icon"><i class="fa fa-angle-down"></i></span>'
        s += '<ul id="dropdown-address" class="dropdown-address">'
        s += '</ul>'
        s += '</div > '
        //設置年
        ...
        for (var i = 0; i < 4; i++) {
            ...
            for (var l = 0; l < 3; l++) {
                ...
                if (month == renderMonth) {
                    s += '<td title="' + year + '88888年' + (renderMonth + 1) + '月" class="calendar-cell calendar-thisMonth">';
                }
                else {
                    s += '<td title="' + year + '9999999年' + (renderMonth + 1) + '月" class="calendar-cell">';
                }
        ...
        for (var i = 0; i < 6; i++) {
            ...
            for (var l = 0; l < 7; l++) {
                ...
                if (renderDate.getMonth() < newDate.getMonth()) {
                    s += '<td title="' + year + '1111111年' + month + '月' + date + '日" class="calendar-cell calendar-last-month-cell">';
                }
                else if (renderDate.getMonth() > newDate.getMonth()) {
                    s += '<td title="' + year + '22222222年' + month + '月' + date + '日" class="calendar-cell calendar-next-month-cell">';
                }
                else if (date == nowNum) {
                    s += '<td title="' + year + '333333年' + month + '月' + date + '日" class="calendar-cell calendar-today">';
                }
                //鼠标移动显示
                else {
                    s += '<td title="' + year + '4444444年' + month + '月' + date + '日" class="calendar-cell">';
                }
                s += '<div class="calendar-date">';
                //日期
